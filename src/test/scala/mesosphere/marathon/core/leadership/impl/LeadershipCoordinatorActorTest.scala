...
class LeadershipCoordinatorActorTest extends MarathonSpec {
  ...
  private[this] var coordinatorRef: TestActorRef[LeadershipCoordinationActor] = _

  before {
    actorSystem = ActorSystem()
    whenLeader1Probe = TestProbe()
    whenLeader2Probe = TestProbe()
    coordinatorRef = TestActorRef(LeadershipCoordinationActor.props(Set(whenLeader1Probe.ref, whenLeader2Probe.ref)))
